...
int predict()
{
	...
	for(i=0;i<2;i++)
	{
		...
		//	probabilities[i]+=log(training_feats[i].rows[j].num_on_sets_likelyhoods[curr.rows[j].num_on_sets%4]);
		...
		//	probabilities[i]+=log(training_feats[i].columns[j].num_on_sets_likelyhoods[curr.columns[j].num_on_sets%4]);
...
void calculate_likelyhoods(int num_training_instances)
{
	...
	for(i=0;i<2;i++)
	{
		...
		for(j=0;j<66;j++)
		{
			FaceRowFeats * tempRow=&training_feats[i].rows[j];
			for(k=0;k<4;k++)
			{
		...
		for(j=0;j<60;j++)
		{
			FaceColumnFeats * tempColumn=&training_feats[i].columns[j];
			for(k=0;k<4;k++)
			{
...
void extract_features(FILE * imagesfd)
{
	...
	for(i=0;i<66;i++)
	{
		...
		curr.rows[i].num_on_sets=num_sets_line%10;
	}
	curr.avg_width=(int)widthsum/66;
	...
	for(i=0;i<60;i++)
	{
		...
		curr.columns[i].num_on_sets=num_sets_line%10;
	}
	curr.avg_height=(int)heightsum/60;
